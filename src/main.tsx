...
import { PrisonersDilemmaGame, Move, leaderboard, type GameJSON } from './game.js'; // Use .js extension
...
const initialGameState: GameJSON | null = null;
...
Devvit.addCustomPostType({
  ...
  render: () => {
    ...
    const [gameState, setGameState] = useState<GameJSON | null>(initialGameState);
    const [gameMode, setGameMode] = useState<"none" | "bot" | "player">("none");
    const [roundResult, setRoundResult] = useState<string>("");
    const [gameOver, setGameOver] = useState<boolean>(false);
    const [leaderboardState, setLeaderboardState] = useState<Record<string, number>>({ ...leaderboard });

    // Function to start a new game.
    const startGame = (mode: "bot" | "player") => {
      setGameMode(mode);
      let newGame: PrisonersDilemmaGame;
      if (mode === "bot") {
    ...
    const handleMove = (move: Move) => {
      ...
      const resText =
        `You chose ${result.playerMove === "C" ? "Cooperate" : "Betray"} | ` +
        ...
        <vstack gap="medium" alignment="center middle" padding="medium">
          ...
          {gameOver && gameState && (
            <>
              ...
              <text size="medium">
                Final Score: You {gameState.playerScore} - Opponent {gameState.opponentScore}
              </text>
              <text size="medium">
...
Devvit.addMenuItem({
  ...
  location: "subreddit",
  onPress: async (_event, context: any) => {
    ...
    const post = await context.reddit.submitPost({
      ...
      preview: (
        <vstack>
          <text>Loading interactive post...</text>
        </vstack>
      ),
    });
    // Use the UI methods provided in the context (cast as any to avoid type errors).
    (context.ui as any).showToast(`Submitted interactive post to r/${currentSubreddit.name}`);
    (context.ui as any).navigateTo(post);
  },
});
